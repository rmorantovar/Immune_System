...
def main():
	...
	fig, ax = plt.subplots(figsize=(8*1.62, 8), gridspec_kw={'left':0.15, 'right':.95, 'bottom':.08, 'top': 0.95})

	N = np.ones_like(time_array)
	Cells = np.ones_like(time_array)*1e12
	for i_t, t in enumerate(time_array[:-1]):
		N[i_t+1] = N[i_t] + (lamA*N[i_t]*(1-N[i_t]/1e12) - delta0*N[i_t])*dT

	ax.plot(time_array, N)
	...
	fig.savefig(output_plot +f'/VL_L0-{int(L0/10**int(np.log10(L0)))}e{int(np.log10(L0))}'+'_N_epi-'+str(N_epi)+'_null.pdf')
	plt.close(fig)
