...
for i_model, energy_model in enumerate(energy_models):
    ...
    if data_ready:
        ...
        df.to_csv(Text_files_path + 'E_model/data_' + energy_model + '.csv')
        df2.to_csv(Text_files_path + 'E_model/data_2_' + energy_model + '.csv')
        df_betas.to_csv(Text_files_path + 'E_model/data_betas_' + energy_model + '.csv')
    else:
        beta_rs = []
        vars_l = []
        good_ls = []
        ...
        for l in tqdm(ls):  
            ...
            for a in range(n_ens):
                ...
                if((K_r > 5e-8/1.5) & (K_r < 5e-8*1.5)): #### K^* condition ####
                    #print('motif found!')
                    Es_average+=Es[:-1]
    ...
    data2d2 = np.histogram2d(good_ls, np.sqrt(vars_l), bins = [ls, np.linspace(0, 2, 80)])
    
    ls_data = data2d[1][:-1] + np.diff(data2d[1])
    alphas_data = data2d[2][:-1] + np.diff(data2d[2])

    vars_data = data2d2[2][:-1] + np.diff(data2d2[2])

    df = pd.DataFrame(data = data2d[0], index = ls_data, columns = ['%.2f'%i for i in alphas_data])
    df2 = pd.DataFrame(data = data2d2[0], index = ls_data, columns = ['%.2f'%i for i in vars_data])
    df_betas = pd.DataFrame(data = data_beta_r[0], index = data_beta_r[1][:-1])

    df.to_csv(Text_files_path + 'E_model/data_' + energy_model + '.csv')
    df2.to_csv(Text_files_path + 'E_model/data_2_' + energy_model + '.csv')
    df_betas.to_csv(Text_files_path + 'E_model/data_betas_' + energy_model + '.csv')
    
    #sns.heatmap(df, ax = ax6, cmap = cmaps_model[i_model])
    #sns.heatmap(df2, ax = ax7, cmap = cmaps_model[i_model])

    print(np.shape(data2d[0]), np.shape(alphas_data), np.shape(ls_data))
    ax6.contourf(alphas_data, ls_data, data2d[0], cmap = cmaps_model[i_model])
    ax7.contourf(vars_data, ls_data, data2d2[0], cmap = cmaps_model[i_model])
