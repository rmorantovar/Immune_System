...
def main():
	...
	for DDE in [0, 1, 2]:
		for index, row in WTs.iterrows():
			...
			axN_A.plot(time_array, N_AMemory, color = my_purple, lw = 4, alpha = .8 - .2*DDE, ls = '-', label = r'$%d k_BT$'%DDE)
	...
	figN_A.savefig(output_plot + '/N_A_t_L0-%de%d_p-%.1f_pmem_%.1f.pdf'%(int(L0/10**int(np.log10(L0))), int(np.log10(L0)), p, pmem))
