...
def main():
	...
	if new:
		...
		for a1, antigen1 in enumerate(tqdm(antigens)):
			...
			try:
				...
				sera = run_essay_Ab_time(data1, 'xxxx', 0, antigen1, a1+1, antigen1, a1+1, energy_model, N_epi, N_ens, l, p, lamA, lamB, C, dT, beta_r, time_array, sera)
				...
				sera = run_essay_Ab_time(data2, antigen1, a1+1, antigen1, a1+1, antigen1, a1+1, energy_model, N_epi, N_ens, l, p, lamA, lamB, C, dT, beta_r, time_array, sera)
		...
		sera_df.to_csv(root_dir + pars_dir_1 + pars_dir_2 + '/T_time.csv', index = False)


	fig11, ax11 = plt.subplots(figsize=(5*1.62, 5), gridspec_kw={'left':0.12, 'right':.9, 'bottom':.1, 'top': 0.96})
	fig21, ax21 = plt.subplots(figsize=(5*1.62, 5), gridspec_kw={'left':0.12, 'right':.9, 'bottom':.1, 'top': 0.96})

	fig12, ax12 = plt.subplots(figsize=(5*1.62, 5), gridspec_kw={'left':0.12, 'right':.9, 'bottom':.1, 'top': 0.96})
	fig22, ax22 = plt.subplots(figsize=(5*1.62, 5), gridspec_kw={'left':0.12, 'right':.9, 'bottom':.1, 'top': 0.96})

	sera_df = pd.read_csv(root_dir + pars_dir_1 + pars_dir_2 + '/T_time.csv')
	range_betas = np.max(sera_df['beta'])-np.min(sera_df['beta'])
	sera_a1 = sera_df.loc[sera_df['past id']==0]
	print(np.min(sera_df['beta']), np.max(sera_df['beta']), range_betas)
	...
	# 	beta_r, E_r, Kd_r = get_repertoire_properties(betas, Q0, Es, dE, L0)
		...
		Z1_1_homo = np.array(sera_a1.loc[(sera_a1['current id']==a1+1) & (sera_a1['test id']==a1+1),'T_1'].apply(literal_eval).iloc[0])
		Z2_1_homo = np.array(sera_a2.loc[(sera_a2['current id']==a1+1) & (sera_a2['test id']==a1+1),'T_1'].apply(literal_eval).iloc[0])
		Z1_2_homo = np.array(sera_a1.loc[(sera_a1['current id']==a1+1) & (sera_a1['test id']==a1+1),'T_2'].apply(literal_eval).iloc[0])
		Z2_2_homo = np.array(sera_a2.loc[(sera_a2['current id']==a1+1) & (sera_a2['test id']==a1+1),'T_2'].apply(literal_eval).iloc[0])
	...
	# 	# 	Z2_hete = sera_a2.loc[(sera_a2['current id']==a2+1) & (sera_a2['test id']==a2+1),'Z'].apply(literal_eval).iloc[0]
		...
		ax11.plot(time_array, Z1_1_homo, color = colors[int(50*(beta1-0.5)/3)], alpha = .4, marker = '')
		...
		ax21.plot(time_array, Z2_1_homo, color = colors[int(50*(beta2-0.5)/3)], alpha = .4, marker = '')
		...
		ax12.plot(time_array, Z1_2_homo, color = colors[int(50*(beta1-0.5)/3)], alpha = .4, marker = '')
		# Z_i_min = Z1_homo[Z1_homo>0][0]
		# Z_i_max = Z1_homo[-1]
		# ax1.plot(time_array[(Z1_homo>100*Z_i_min) & (Z1_homo<0.2*Z_i_max)], Z1_homo[(Z1_homo>100*Z_i_min) & (Z1_homo<0.2*Z_i_max)], color = colors[int(50*(beta1-0.5)/3)], alpha = .2, marker = '.', ls = '')

		ax22.plot(time_array, Z2_2_homo, color = colors[int(50*(beta2-0.5)/3)], alpha = .4, marker = '')
		# Z_i_min = Z2_homo[Z2_homo>0][0]
		# Z_i_max = Z2_homo[-1]
		min_time = 3.823823823823824
	...
	my_plot_layout(ax = ax11, xscale='linear', yscale= 'log', ticks_labelsize= 24, x_fontsize=30, y_fontsize=30)
	# ax1.legend(fontsize = 18, title_fontsize = 22, loc = 2, title = r'$\mathrm{Infection}$')
	# ax1.set_xlim(left = 2e-6, right = 2e2)
	ax11.set_ylim(bottom = 1e13, top = 1e21)
	# ax1.set_xticks(np.linspace(0, time_array[-1]*(N_inf-1), int(N_inf/8)))
	# ax1.set_xticklabels([r'$%d$'%i for i in range(1, N_inf+1, 8)])
	fig11.savefig('../../../Figures/memory/DeltaZ/T1_1_t_Panel_'+energy_model+'_L0-'+str(L0)+'_N_ant-'+str(N_ant)+'_N_ens-'+str(N_ens)+'_N_evo-'+str(N_evo)+'.pdf')

	my_plot_layout(ax = ax21, xscale='linear', yscale= 'log', ticks_labelsize= 24, x_fontsize=30, y_fontsize=30)
	# ax2.legend(fontsize = 18, title_fontsize = 22, loc = 2, title = r'$\mathrm{Infection}$')
	# ax2.set_xlim(left = 2e-6, right = 2e2)
	ax21.set_ylim(bottom = 1e13, top = 1e21)
	# ax2.set_xticks(np.linspace(0, time_array[-1]*(N_inf-1), int(N_inf/8)))
	# ax2.set_xticklabels([r'$%d$'%i for i in range(1, N_inf+1, 8)])
	fig21.savefig('../../../Figures/memory/DeltaZ/T2_1_t_Panel_'+energy_model+'_L0-'+str(L0)+'_N_ant-'+str(N_ant)+'_N_ens-'+str(N_ens)+'_N_evo-'+str(N_evo)+'.pdf')

	my_plot_layout(ax = ax12, xscale='linear', yscale= 'log', ticks_labelsize= 24, x_fontsize=30, y_fontsize=30)
	# ax1.legend(fontsize = 18, title_fontsize = 22, loc = 2, title = r'$\mathrm{Infection}$')
	# ax1.set_xlim(left = 2e-6, right = 2e2)
	ax12.set_ylim(bottom = 1e13, top = 1e21)
	# ax1.set_xticks(np.linspace(0, time_array[-1]*(N_inf-1), int(N_inf/8)))
	# ax1.set_xticklabels([r'$%d$'%i for i in range(1, N_inf+1, 8)])
	fig12.savefig('../../../Figures/memory/DeltaZ/T1_2_t_Panel_'+energy_model+'_L0-'+str(L0)+'_N_ant-'+str(N_ant)+'_N_ens-'+str(N_ens)+'_N_evo-'+str(N_evo)+'.pdf')

	my_plot_layout(ax = ax22, xscale='linear', yscale= 'log', ticks_labelsize= 24, x_fontsize=30, y_fontsize=30)
	# ax2.legend(fontsize = 18, title_fontsize = 22, loc = 2, title = r'$\mathrm{Infection}$')
	# ax2.set_xlim(left = 2e-6, right = 2e2)
	ax22.set_ylim(bottom = 1e13, top = 1e21)
	# ax2.set_xticks(np.linspace(0, time_array[-1]*(N_inf-1), int(N_inf/8)))
	# ax2.set_xticklabels([r'$%d$'%i for i in range(1, N_inf+1, 8)])
	fig22.savefig('../../../Figures/memory/DeltaZ/T2_2_t_Panel_'+energy_model+'_L0-'+str(L0)+'_N_ant-'+str(N_ant)+'_N_ens-'+str(N_ens)+'_N_evo-'+str(N_evo)+'.pdf')
