...
def main():
	...
	for a1, antigen1 in enumerate(antigens[:]):
		if a1==0:
			...
			figN_A, axN_A = plt.subplots(figsize=(5*1.62, 5), gridspec_kw={'left':0.10, 'right':.95, 'bottom':.1, 'top': 0.95})
			...
			dataMemory = pd.read_csv(root_dir + pars_dir_1 + pars_dir_2 + '/%d'%(a1+1) + '/%d'%(a1) + '/activated_repertoire.csv')#, converters={"N_t": literal_eval})
			E_ms = []
			...
			for i in tqdm(range(N_ens)):
				...
				if i%20==0:
					axZ.plot(time_array, ZNaive_i, color = my_blue, lw = .2, alpha = .2, ls = '-', marker = '.', ms = 0.5)
				...
				if i%20==0:
					axZ.plot(time_array, ZMemory_i, color = my_purple, lw = .2, alpha = .2, ls = '-', marker = '.', ms = 0.5)
			...
			axZ.plot(time_array, ZNaive, color = my_blue, lw = 3, alpha = .9, ls = '-', label = r'$\textrm{Naive}$')
			axZ.plot(time_array, ZMemory, color = my_purple, lw = 3, alpha = .9, ls = '-', label = r'$\textrm{Memory}$')

			lamZ = np.max([lamB, lamA/p*(beta_r-1)])
			print(lamZ)

			Z_mf = Kstep**(1/v)/(Kd_r)**(1+1/v)*np.exp(lamZ*(time_array - t0))
			axZ.plot(time_array[(time_array<7) & (time_array>t0)], Z_mf[(time_array<7) & (time_array>t0)], alpha = .5, color = my_blue, lw = 2, ls = '--')#, label = r'$\textrm{Mean-field}\, \textrm{approx}$')
			
			Z_mf = 0.1*C*Kstep**(1/v)/(Kd_r)**(1+1/v)*np.exp(lamA*(time_array - t0))
			axZ.plot(time_array[(time_array<4) & (time_array>t0-1)], Z_mf[(time_array<4) & (time_array>t0-1)], alpha = .5, color = my_purple, lw = 2, ls = '--')#, label = r'$\textrm{Mean-field}\, \textrm{approx}$')
			
			# axZ.hlines(1/Kd_r, 0, T, color = 'k', ls = '--')

			my_plot_layout(ax = axZ, xscale='linear', yscale= 'log', ticks_labelsize= 24, x_fontsize=30, y_fontsize=30)
			# axZ.set_xticks([])
			# axZ.set_yticks([])
			...
			my_plot_layout(ax = axN_A, xscale='linear', yscale= 'log', ticks_labelsize= 24, x_fontsize=30, y_fontsize=30)
			# axN_A.set_xticks([])
			# axN_A.set_yticks([])
			axN_A.set_ylim(bottom = 1e4, top = 2e13)
			axN_A.set_xlim(left = 1, right = 8)
			axN_A.legend(fontsize = 18, loc = 2)
			figN_A.savefig(output_plot + '/N_A_%d_t.pdf'%(a1+1))
