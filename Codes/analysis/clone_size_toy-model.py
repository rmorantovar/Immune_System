...
for j, d in enumerate(ds):
	...
	for n, N in enumerate(Ns):
		for i, beta in enumerate(betas):
			...
			#print(clone_sizes)
			density = False
			clone_size_distribution = np.histogram(clone_sizes, bins = np.logspace(np.log10(np.min(clone_sizes)),np.log10(np.max((clone_sizes))),20), density = density)
			...
			if density==False:
				ax.plot(clone_size[:], clone_size_counts[:], linestyle = '-', marker = markers_beta[i], ms = 8, linewidth = 2, color = colors_d[j][n+2])
