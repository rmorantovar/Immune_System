...
#
# Equation solved:
#   1 - P_free = ∫ [ A0(k) * k_er_plus * k_m_plus ] / [ (k_step + k) *
#                 (k_er_minus + M_tot * P_free * k_m_plus * (1 - k/(k_step + k)) ) ] * Omega(k) dk
#
...
def make_omega(k, dist='lognormal', **kwargs):
...
def build_integrand(k, P_free, M_tot, k_m_plus, k_step, k_er_plus, k_er_minus, Omega, A0):
    """Compute integrand(k) for the RHS at a given P_free.
    integrand(k) = [A0(k) * k_er_plus *  * M_tot * P_free] / [ (k_step + k) *
                    (k_er_minus + M_tot * P_free * k_m_plus * (k_step/(k_step + k)) ) ] * Omega(k)
    """
    M_free = M_tot * P_free
    numerator = A0 * k_er_plus * k_m_plus * P_free
    denom_term = k_er_minus + M_tot * P_free * k_m_plus * (k_step / (k_step + k))**1
    integrand_vals = (numerator / ((k_step + k) * denom_term)) * Omega * 1e7
    return integrand_vals


def solve_pfree(k_grid, Omega, A0, M_tot, k_m_plus, k_step, k_er_plus, k_er_minus, bracket=(1e-6, 1-1e-6)):
    ...
    def self_consistency(P_free):
        vals = build_integrand(k_grid, P_free, M_tot, k_m_plus, k_step, k_er_plus, k_er_minus, Omega, A0)
...
def parse_args(argv=None):
    ...
    ap.add_argument('--k_m_plus', type=float, default=1e-6, help='MHC binding rate (default: 1.0)')
    ap.add_argument('--k_step', type=float, default=.01, help='Stepping rate (default: 1.0)')
    ap.add_argument('--k_er_plus', type=float, default=1e2, help='ER entry rate (default: 1.0)')
    ap.add_argument('--k_er_minus', type=float, default=1e-5, help='ER exit rate (default: 0.1)')

    # k-grid
    ap.add_argument('--k_min', type=float, default=1e-3, help='min k_m_minus (default: 0.01)')
    ap.add_argument('--k_max', type=float, default=1e2, help='max k_m_minus (default: 10.0)')
    ...
    ap.add_argument('--mu', type=float, default=8.0, help='lognormal mu')
    ap.add_argument('--sigma', type=float, default=2., help='lognormal sigma')
    ...
    ap.add_argument('--A0_a0', type=float, default=1e-6, help='A0 constant value (for constant)')
...
def main(argv=None):
    ...
    if args.dist == 'lognormal':
        Omega = make_omega(k, dist='lognormal', mu=args.mu, sigma=args.sigma)
    ...
    ax.plot(k, Omega*1e7, marker='.', label='Omega(k)', color='blue')
    ...
    print(np.sum(Omega[:-1]*np.diff(k)))
    ...
    for k_er_minus in tqdm(np.logspace(-4, 0, num=5)):
        P_free_values = []
        M_tot_value = np.logspace(1, 8, num=20)
        for M_tot in M_tot_value:
            # print(f'Solving for M_tot = {M_tot:.2f}...')
        ...
        # print(f'P_free = {P_free:.6f}')
        # print(f'Bound fraction = {1.0 - P_free:.6f}')
        # print(f'M_free = {args.M_tot * P_free:.6f}')

        
        ax.plot(M_tot_value, np.array(P_free_values), marker='o', label=r'$k_{\mathrm{er}}^{-} = %.1e$'%(k_er_minus))
