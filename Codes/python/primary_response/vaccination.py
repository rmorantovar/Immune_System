...
for immunization in immunizations:
    ...
    #ax2.plot(tGrid[:], np.exp(lamA*tGrid[:]), lw = 5, color = antigen_color)
    for i_D, D in enumerate(Ds):
        results = defaultdict(list)
        for i_r0, r0 in enumerate(r0s):
            for i_N0, N0 in enumerate(N0s):
                ...
                if i_N0 == 1:
                    for i_t in [100, 200, 300, 400, 500]:
                        ax1.plot(xGrid/r0, C[i_t, :], lw = 5, color = colorsD[i_N0], alpha = i_t/(500), label = r'$%.1f$'%(i_t*dt))
                        if i_N0 == 1:
                            ax1.fill_between((xGrid/r0)[xGrid>r0], np.ones_like(C[i_t, xGrid>r0]), C[i_t, xGrid>r0], lw = 5, color = colorsD[i_N0], alpha = .1)
                            ax1.hlines(N0*0.08*np.exp(i_t*dt*growth_rate)/(2*np.pi*D*dt*i_t)**(3/2), 0, 1.5*np.sqrt(2*D*i_t*dt)/r0, color = 'black', ls = '--')
                
                ax2.plot(tGrid[:], NAeff, lw = 4, ls = '-', label = r'$%.1f$'%(alpha), color = colorsD[i_N0])#, color = antigen_color)
                # ax2.vlines(tstar, 1, 1e12, lw = 1, ls = '--', color = 'k')
                ax2.hlines(N0, 0, 8, lw = 2, ls = '--', color = colorsD[i_N0])
    ...
    ax1.set_ylim(bottom = 1e0, top = np.max([1.5e16]))
    ...
    ax2.set_ylim(bottom = 1e0, top = 1.5e16)
