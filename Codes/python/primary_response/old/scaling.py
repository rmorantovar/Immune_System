...
D0 = 1e-3
Ds = np.logspace(np.log10(0.03*D0), np.log10(20*D0), 10)
...
for immunization in immunizations:
    ...
    # print(results_db)
    alpha_array = np.logspace(np.log10(0.045), np.log10(4), 50)
    ...
    for i_r0, r0 in enumerate(r0s):
        ...
        ax_lam.plot(data_N['alpha'], (data_N['lamA']/lamA), lw = 5, ls = ''
            , marker = markers[i_r0], ms = 8, color = my_purple, label = r'$%.1f$'%(r0))
        
        ax_t.plot(data_N['alpha'], data_N['t']/((1/lamA)*np.log(Nstar)), lw = 5, ls = ''
            , marker = markers[i_r0], ms = 8, color = my_purple, label = r'$%.1f$'%(r0))
    
    
    ax_lam.plot(alpha_array, 1+1/(alpha_array**(2)*(2*np.log(Nstar) + 4*np.log(alpha_array) - np.log(2/3.14)) + 1)
        , color = my_purple2, lw = 5, alpha = .6)
    # ax_lam.vlines(0.273548, 0, 1)
    
    
    ax_t.plot(alpha_array, 1+1/(2*np.log(Nstar)*alpha_array**(2)) + 2*np.log(alpha_array)/np.log(Nstar) - np.log(2/3.14)/(2*np.log(Nstar))
        , color = my_purple2, lw = 5, alpha = .6)
...
for immunization in immunizations:
    ...
    #alpha_array = np.logspace(np.log10(1.95e-1), np.log10(3.5e-1), 50)
    ...
    for i_r0, r0 in enumerate(r0s):
        ...
        ax_lam.plot(data_r0['alpha'], (data_r0['lamA']/lamA), lw = 5, ls = ''
            , marker = markers[i_r0], ms = 8, color = my_purple, label = r'$%.1f$'%(r0))
        ax_lam.plot(alpha_array, D/r0**2*alpha_array**(-4)/lamA
        , color = my_purple2, lw = 5, alpha = .6)
        
        ax_t.plot(data_r0['alpha'], data_r0['t']/((1/lamA)*np.log(Nstar)), lw = 5, ls = ''
            , marker = markers[i_r0], ms = 8, color = my_purple, label = r'$%.1f$'%(r0))
        ax_t.plot(alpha_array, alpha_array**2*r0**2/(2*D)*lamA/np.log(Nstar)
        , color = my_purple2, lw = 5, alpha = .6)
...
ax_lam.legend(fontsize = 20, title_fontsize = 24, loc = 0, title = r'$r_0$')#, labels = [r'$%.1e$'%i for i in N0s])
fig_lam.savefig('../../../Figures/primary_immune_response/1_Dynamics/antigen_concentration/lam.pdf')

my_plot_layout(ax = ax_t, xscale='log', yscale= 'log', ticks_labelsize= 30, x_fontsize=30, y_fontsize=30 )
#ax_t.set_xlim(left = 0, right = 7)
ax_t.set_ylim(bottom = -0.5, top = 11)
ax_t.set_yticks(range(1, 12, 3))
ax_t.legend(fontsize = 20, title_fontsize = 24, loc = 0, title = r'$r_0$')#, labels = [r'$%.1e$'%i for i in N0s])
fig_t.savefig('../../../Figures/primary_immune_response/1_Dynamics/antigen_concentration/t.pdf')
plt.close()
