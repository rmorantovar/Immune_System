...
for immunization in immunizations:
    ...
    D0 = 3e-4
    for i_D, D in enumerate([D0, 10*D0, 100*D0]):
        ...
        ax2.plot(tGrid[:], NAeff, lw = 4, ls = '-', label = r'$%.0e$'%(D), color = colorsD[i_D])#, color = antigen_color)
    ...
    ax2.set_xlim(left = 0, right = 7)
    ax2.set_ylim(bottom = 1e0, top = 1e12)
    ax2.legend(fontsize = 20, title_fontsize = 24, title = r'$D$', loc = 0)
