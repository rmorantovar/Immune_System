...
for i_model, energy_model in enumerate(energy_models):
    ...
    K_rs = []
    K_ms = []
    n_T = 100000  # or whatever you chose
    Es_average = None
    rho_average = None
    counter = 0
    ...
    for l in tqdm(ls):  
        ...
        for a in range(n_ens):
            ...
            for j in range(5):
                ...
                #--------------------------Entropy function--------------------------
                Es, dE, rho_0, betas = calculate_Q0(0.005, 100, n_T, Motif_new, E_m_prime - E_0_prime + E_o, l)
                beta_r, E_r, K_r = get_repertoire_properties(betas, rho_0, Es, dE, L_0)
                if((K_r > 8e-8/1.5) & (K_r < 8e-8*1.5)): #### K^* condition ####
                    #print('motif found!')
                    if Es_average is None:
                        Es_average = np.zeros_like(Es[:-1])
                        rho_average = np.zeros_like(rho_0)
                    Es_average+=Es[:-1]
    ...
    df.to_csv(Text_files_path + 'exponential_proofreading/E_model/All_data_' + energy_model + '.csv')
    f = open(Text_files_path + 'exponential_proofreading/E_model/trajectories_'+energy_model+'.pkl', 'wb')
