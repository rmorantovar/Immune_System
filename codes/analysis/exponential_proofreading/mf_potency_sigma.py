...
if __name__ == '__main__':
    ...
    base = dict(N_A0=1.0, delta_A=6.0, lambda_A = 6.,
                ...
                hill=1.0, h0=1e-2, beta_star=2.5, K_T = 1e5,
    ...
    ax_potency.plot(sigmas, potencies, label=r'$\mathrm{Potency}$', color='tab:blue', lw = 2)
    ax_potency.plot(sigmas, Byields, label=r'$\mathrm{Yield}$', color='tab:red', lw = 2)

    # Formatting
    ax_potency.set_xlabel(r'$\sigma$')
    ...
    plt.tight_layout()
    fig_potency.savefig(os.path.join(output_plot, f'potency_sigma.pdf'), dpi=150, bbox_inches='tight')