...
if __name__ == '__main__':
    ...
    # Default parameters
    base = dict(N_A0=1.0, lambda_A = 6.0, delta_A=4.0, sigma= 2.0,
    ...
    h0s = np.logspace(-2, np.log10(base['b0']), 10)
    ...
    for i_h0, h0 in enumerate(h0s):
        ...
        for i_m, memory in enumerate([0]):
            ...
            base['memory'] = memory
            figs, axes = plt.subplots(3, 3, figsize=(24, 20))
            ...
            # (e) B
            axi = axes[1, 1]
            N_B = (res['N_Bo'] + res['N_Ba'])*res['weights'][:, None]
            N_B[N_B<2.0] = 0
            # activated = N_B > 1.5
            ...
            print(res['weights'][-1])
            ax1 = axi.semilogy(t, [np.sum(res['weights'][0: L_act[i_t]]) for i_t in range(len(t))], label=label, linewidth = 4)
            ...
            axes[0, 1].set_ylabel(r'$\pi$', fontsize = 16)
            axes[0, 1].set_xticklabels([])
            axes[0, 1].set_ylim(bottom = 1e-4, top = 1e4)
            axes[0, 1].set_xlim(0, T)

            # axes[0, 2].set_xlabel('Time')
            axes[0, 2].set_ylabel(r'$\lambda_B/b_o$', fontsize = 16)
            axes[0, 2].set_xticklabels([])
            axes[0, 2].set_xlim(0, T)
            axes[0, 2].tick_params(labelsize=14)

            # axes[3].set_xlabel('Time')
            axes[1, 0].set_ylabel('T cells', fontsize = 16)
            axes[1, 0].axhline(1.0, color='k', linestyle=':', alpha=0.5)
            axes[1, 0].set_xticklabels([])
            axes[1, 0].set_xlim(0, T)
            axes[1, 0].set_yscale('log')
            axes[1, 0].set_ylim(bottom = 0.1*p.N_T0, top = 1e5*p.N_T0)
            ...
            axes[1, 2].set_ylabel('$\mathrm{Lineages}$', fontsize = 16)
            # axes[1, 2].set_xticklabels([])
            axes[1, 2].set_xlabel('Time', fontsize = 16)
            axes[1, 2].set_ylim(bottom = 5e-1, top = 1e4)
            axes[1, 2].set_xlim(0, T)
            axes[1, 2].set_yscale('log')
            axes[1, 2].tick_params(labelsize=14)

            # axes[2, 0].set_xlabel('Time')
            axes[2, 0].set_ylabel('Potency, $Z$', fontsize = 16)
            # axes[2, 0].set_xticklabels([])
            axes[2, 0].set_xlabel('Time', fontsize = 16)
            axes[2, 0].set_ylim(bottom = 5e-1, top = 1e8)
