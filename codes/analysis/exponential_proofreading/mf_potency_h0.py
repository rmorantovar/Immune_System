...
if __name__ == '__main__':
    ...
    ax_potency.plot(h0s, potencies, label=r'$\mathrm{Potency}$', color='tab:blue', lw = 2)
    ax_potency.plot(h0s, Byields, label=r'$\mathrm{Yield}$', color='tab:red', lw = 2)


    # Formatting
    ax_potency.set_xlabel(r'$h_0$')
    # ax_potency.set_ylabel('Frequency')
    ax_potency.legend(fontsize=7)
    ax_potency.set_yscale('log')
    ax_potency.set_xscale('log')
    ax_potency.tick_params(axis='both', which='major', labelsize=14)
    ax_potency.legend(fontsize=14)
    plt.tight_layout()
    fig_potency.savefig(os.path.join(output_plot, f'potency_h0.pdf'), dpi=150, bbox_inches='tight')