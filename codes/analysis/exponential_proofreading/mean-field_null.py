...
if __name__ == '__main__':
    ...
    colors_sim = [my_green, my_brown, my_purple2, my_gold, my_brown, my_blue, my_green, 'tab:orange', my_purple, my_cyan]
    styles_sim = [':', '-', ':', '-.', '-', '--', ':', '-.', '-', '--']
    N_ensemble = 1

    base['h0'] = np.array([base['b0']/1000.])
    pi_stars = (base['b0']/base['h0'])**(1/base['hill'])
    ...
    for i_null, null in enumerate([0, 1]):
        print(f"... for null={null}")
        for i_m, memory in enumerate([0, 1]):
            output_plot = f'/Users/robertomorantovar/Dropbox/_Documents/Research/Projects/Immune_System/_Repository/Figures/{project}/{model}/{submodel}/{subproject}/{subsubproject}/null_{null}/memory_{memory}/'
            ...
            for i_ensemble in range(N_ensemble):
                if null == 0:
                    ...
                    if p.memory == 0:
                        ...
                        res = run_simulation_null(p=p, t_span=(0, T), mode='grid', memory_seed=formed_memory)
            ...
            if null == 0:
                ...
                Z_B_total = compute_potency_null_t(res)
                Z_B = N_B * res['weights'][:, None] * np.exp(-res['DG'][:, None]) * res['a']
                Z_B[N_B <= 2.0] = np.nan

            ax1 = ax_Z.plot(t, Z_B_total, linewidth = 4, color=colors_sim[i_m], label=label)
            ax_Z_shared.plot(t[Z_B_total>0], Z_B_total[Z_B_total>0], linewidth = 5, color=colors_sim[i_null], label=label, ls = styles_sim[i_m])
            if p.memory==0:
                ax_Z_shared.scatter(t[Z_B_total>=p.Z_c][0], Z_B_total[Z_B_total>=p.Z_c][0], facecolor='white', edgecolor = colors_sim[i_null], linewidth = 2, s=250, zorder=10)
            else:
                ax_Z_shared.scatter(t[Z_B_total>=p.Z_c][0], Z_B_total[Z_B_total>=p.Z_c][0], facecolor=colors_sim[i_null], edgecolor = 'k', linewidth = 2, s=250, zorder=10)
            ...
            ax_pb_shared.plot(t, pb, label=label, linewidth = 4, color=colors_sim[i_null], ls = styles_sim[i_m])
