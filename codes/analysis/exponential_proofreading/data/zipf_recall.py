...
output_plot = '/Users/robertomorantovar/Dropbox/_Documents/Research/Projects/Immune_System/_Repository/Figures/'+project+'/'+ subproject + '/mesin2020/zipf/recall'
...
for i_ph, ph in enumerate(phs):
    ...
    ax_r2.plot(range(1, max_rank_eff+1), x_avg, color = color, markerfacecolor="None", ms = 12, alpha = 1, ls = '', marker = markers_ph[i_ph], label = r'$%.2f$'%(np.mean(zetas)) + ' ; ' + ph)
    ax_r2.plot(np.arange(1, max_rank_eff + 1), np.arange(1, max_rank_eff + 1)**(-np.mean(zetas)), color = color, alpha = .8, lw = 3)

    parts = ax_zeta2.violinplot([zetas], positions=[i_ph], showmeans=True, showextrema=False)
    for i, body in enumerate(parts['bodies']):
        body.set_facecolor(color)
        body.set_edgecolor('black')
        body.set_alpha(0.5)
    parts['cmeans'].set_color(color)
    ax_zeta2.scatter(np.random.normal(i_ph, 0.04, len(zetas_mice)), zetas_mice, color=color, edgecolor='k', s=80, alpha=.5)
    ax_zeta2.scatter(i_ph, np.mean(zetas_mice), color=color, edgecolor='k', s=150)
...
ax_zeta2.set_xticks(range(len(phs)), ['']*len(phs))
