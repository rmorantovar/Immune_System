...
def main():
	...
	data_dir = f"/Users/robertomorantovar/Dropbox/Research/Immune_system/memory/PS/N_ens_{N_ens}_N_evo_{N_evo}_N_inf_{N_inf}_N_epi_{N_epi}_L0_{L0}_l_{l}_p_{p}/"
	...
	antigens = [x[0][-16:] for x in os.walk(data_dir)][1:]
	print(antigens)
	N_ant = len(antigens)

	t12s = []
	Z12s = []
	for a, ant in enumerate(antigens):
		for infection in tqdm(range(1, N_inf + 1)):
			data = pd.read_csv(data_dir + ant + '/act_pop_'+ str(infection) + '.csv')
			Es_update = []
			...
			t12 = time[Z<Z[-1]/2][-1]
			Z12s.append(Z12)
			t12s.append(t12 - (infection-1)*time_array[-1])
	...
	ax_times.scatter(t12s, Z12s, s = 25)
	...
	# fig.savefig('../../../Figures/memory/DeltaZ/Z_t_'+energy_model +'_'+str(N_evo)+'.pdf')

	my_plot_layout(ax = ax_times, xscale='linear', yscale= 'log', ticks_labelsize= 24, x_fontsize=30, y_fontsize=30)
	# ax_times.legend(fontsize = 18, title_fontsize = 22, loc = 2, title = r'$\mathrm{Infection}$')
	ax_times.set_xlim(left = 4, right = 14)
	ax_times.set_ylim(bottom = 1e8, top = 1.5e11)
	...
	fig_times.savefig('../../../Figures/memory/DeltaZ/Zt12_recurrent_'+energy_model + '_'+str(N_evo)+'.pdf')
